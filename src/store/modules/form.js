...
function lookForUnfilledRequiredValues (configurations, obj, ignore_configs_array) {
    let unfilled_required = [];
    configurations.forEach(config => {
        if (obj.local_entity_data[config.id])  // in local_entity_data => is valid config
            return;
        
        if (config.required_in_create_form && !ignore_configs_array.includes(config.id))
...
const actions = {
    ...
    fillLocalEntityData({commit, state, rootState}) {
        ...
        let form = state.form;
        commit('SET_FORM_LOCAL_ENTITY_DATA', fillObjLocalEntityData(state_api.form_config, form));
        commit('SET_FORM_UNFILLED_REQUIRED_VALUES', lookForUnfilledRequiredValues(state_api.form_config, form, []));
        ...
        form.rows.forEach((row, rowIdx) => {
            ...
            commit('SET_ROW_UNFILLED_REQUIRED_VALUES', {
                'rowIdx': rowIdx,
                'data': lookForUnfilledRequiredValues(state_api.rows_config, row, [state_api.rows_config.find(config => config.name === 'Formulario').id])
            });
            ...
            row.sections.forEach((section, sectionIdx) => {
                ...
                commit('SET_SECTION_UNFILLED_REQUIRED_VALUES', {
                    'rowIdx': rowIdx,
                    'sectionIdx': sectionIdx,
                    'data': lookForUnfilledRequiredValues(state_api.sections_config, section, [
                        state_api.sections_config.find(config => config.name === 'Formulario').id,
                        state_api.sections_config.find(config => config.name === 'Fila del formulario').id
                    ])
                });
                ...
                section.fields.forEach((field, fieldIdx) => {
                    ...
                    commit('SET_FIELD_UNFILLED_REQUIRED_VALUES', {
                        ...
                        'sectionIdx': sectionIdx,
                        'fieldIdx': fieldIdx,
                        'data': lookForUnfilledRequiredValues(state_api.fields_config, field, [
                            state_api.fields_config.find(config => config.name === 'Formulario').id,
                            state_api.fields_config.find(config => config.name === 'Sección formulario').id
                        ])
                    })
                })
            })
